...
export default function StaffEdit({ staff, roleOptions = {}, sectionOptions = {}, teams = [] }) {
    const { data, setData, post, processing, errors } = useForm({
        ...
        team_assignments: staff?.team_assignments?.length ? staff.team_assignments : [{ team_id: '', role: '' }],
        _method: 'PUT',
    });
    ...
    const submit = (e) => {
        e.preventDefault();
        post(`/admin/staff/${staff.id}`, { forceFormData: true });
    };

    return (
        <AdminLayout>
            <Head title={`Modifier ${staff?.first_name} ${staff?.last_name}`} />
            <div className="space-y-6 p-4 sm:p-6">
                <div>
                    ...
                    <h1 className="text-2xl font-bold text-foreground">Modifier le staff</h1>
                </div>

                <Card>
                    <CardHeader>
                        <CardTitle>Informations</CardTitle>
                        <CardDescription>Rôle et équipes d’affectation</CardDescription>
                    </CardHeader>
                    <CardContent>
                        <form onSubmit={submit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                {staff?.image && <p className="text-xs text-muted-foreground">Photo actuelle affichée sur le site</p>}
                            ...
                            <div className="border-t pt-6">
                                ...
                                {assignments.map((a, index) => (
                                    <div key={index} className="flex flex-wrap items-end gap-2 mb-2">
                                        ...
                                        <Button type="button" variant="ghost" size="icon" className="h-9 w-9 text-destructive" onClick={() => removeAssignment(index)} disabled={assignments.length <= 1}>
